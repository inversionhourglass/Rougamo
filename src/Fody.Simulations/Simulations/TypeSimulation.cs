...
{
    [DebuggerDisplay("{Ref}")]
    public class TypeSimulation : Simulation, IHost
    {
        ...
        public TypeSimulation(TypeReference typeRef, IHost? host, SimulationModuleWeaver moduleWeaver) : base(moduleWeaver)
        {
        ...
        public virtual IList<Instruction> Cast(TypeReference to)
        {
            ...
            if (to.IsValueType || to.IsGenericParameter) throw new FodyWeavingException($"Cannot convert {Ref} to {to}. Only object types can be converted to value types.");
        ...
        private PropertySimulation? PropertySimulateInner(string propertyName, bool recursion, bool optional)
        {
            if (!_propertySimulations.TryGetValue(propertyName, out var simulation))
            {
                ...
                if (propertyDef == null && !optional) throw new FodyWeavingException($"Cannot find property({propertyName}) from {Def.FullName}");
    ...
    public static class TypeSimulationExtensions
    {
        private static readonly ConcurrentDictionary<Type, Func<TypeReference, IHost?, SimulationModuleWeaver, object>> _Cache = [];

        public static TypeSimulation Simulate(this TypeReference typeRef, SimulationModuleWeaver moduleWeaver) => new(typeRef, null, moduleWeaver);

        public static TypeSimulation Simulate(this TypeReference typeRef, IHost? host, SimulationModuleWeaver moduleWeaver) => new(typeRef, host, moduleWeaver);

        public static T Simulate<T>(this TypeReference typeRef, SimulationModuleWeaver moduleWeaver) where T : TypeSimulation => Simulate<T>(typeRef, null, moduleWeaver);

        public static T Simulate<T>(this TypeReference typeRef, IHost? host, SimulationModuleWeaver moduleWeaver) where T : TypeSimulation
        {
            var ctor = _Cache.GetOrAdd(typeof(T), t =>
            {
                var ctorInfo = t.GetConstructor([typeof(TypeReference), typeof(IHost), typeof(SimulationModuleWeaver)]);
        ...
        {
            if (simulation.Ref is not GenericInstanceType git) throw new FodyWeavingException($"Cannot set generic parameters for {simulation.Ref}, it is not a generic type.");
            if (git.GenericArguments.Count != generics.Length) throw new FodyWeavingException($"Cannot set generic parameters for {simulation.Ref}, given generic parameters [{string.Join(",", generics.Select(x => x.ToString()))}]");
