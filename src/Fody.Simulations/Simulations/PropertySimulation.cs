...
namespace Fody.Simulations
{
    ...
    public class PropertySimulation(TypeSimulation declaringType, PropertyDefinition propertyDef) : Simulation(declaringType.ModuleWeaver), ILoadable, IAssignable
    {
    ...
    public class PropertySimulation<T>(TypeSimulation declaringType, PropertyDefinition propertyDef) : PropertySimulation(declaringType, propertyDef) where T : TypeSimulation
    {
        public new MethodSimulation<T>? Getter { get; } = propertyDef.GetMethod?.Simulate<T>(declaringType);
    }

    public static class PropertySimulationExtensions
    {
