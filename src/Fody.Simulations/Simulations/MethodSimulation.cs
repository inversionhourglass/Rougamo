...
{
    [DebuggerDisplay("{Ref}")]
    public class MethodSimulation : Simulation
    {
        ...
        {
            if (Def.Parameters.Count != arguments.Length) throw new FodyWeavingException($"Parameters count not match of method {Def}, need {Def.Parameters.Count} gave {arguments.Length}");
...
#pragma warning restore CS8619 // Nullability of reference types in value doesn't match target type.
        ...
        {
            if (_genericParaIndexes == null) throw new FodyWeavingException($"[{Def}] Not all method generic parameters are inferred from parameters, you have to specify all the generic parameters manually.");
            return _genericParaIndexes.Select(indexes =>
            {
                var arg = arguments[indexes[0]] ?? throw new FodyWeavingException($"[{Def}] Cannot infer the generic type from a null value at parameter index {indexes}");
                var argType = arg.Type.Ref;
                for (var i = 1; i < indexes.Length; i++)
                {
                    if (argType is not GenericInstanceType git) throw new FodyWeavingException($"[{Def}] Cannot infer the generic type from type {argType}");
